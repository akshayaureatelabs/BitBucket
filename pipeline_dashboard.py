...
class PipelineDashboard:
    ...
    def generate_dashboard(self, days: int = 30) -> str:
        ...
        if metrics["daily"]:
            ...
            fig.add_trace(
                go.Bar(name="Total Runs", x=dates, y=runs, marker_color="lightblue"),
                row=1,
                col=1,
            )
            fig.add_trace(
                go.Bar(
                    name="Successful", x=dates, y=success, marker_color="lightgreen"
                ),
                row=1,
                col=1,
            )
        ...
        if metrics["branches"]:
            ...
            success_rate = [
                (b[2] / b[1] * 100) if b[1] else 0 for b in metrics["branches"][:10]
            ]
            colors = [
                "green" if r > 80 else "orange" if r > 60 else "red"
        ...
        if metrics["steps"]:
            steps = [
                s[0][:30] + "..." if len(s[0]) > 30 else s[0]
                for s in metrics["steps"][:10]
            ]
            durations = [s[2] for s in metrics["steps"][:10]]
            fig.add_trace(
                go.Bar(
                    name="Avg Duration (s)", x=steps, y=durations, marker_color="purple"
                ),
                row=2,
                col=1,
            )
        ...
        if metrics["test_trends"]:
            ...
            fig.add_hline(
                y=self.COVERAGE_THRESHOLD,
                line_dash="dash",
                line_color="red",
                annotation_text=f"Minimum {self.COVERAGE_THRESHOLD}%",
                row=2,
                col=2,
            )
        ...
        if metrics["steps"]:
            steps = [
                s[0][:20] + "..." if len(s[0]) > 20 else s[0] for s in metrics["steps"]
            ]
            success_rate = [
                (s[3] / s[1] * 100) if s[1] else 0 for s in metrics["steps"]
            ]
            colors = [
            ...
            fig.add_trace(
                go.Bar(
                    name="Success Rate %", x=steps, y=success_rate, marker_color=colors
                ),
                row=3,
                col=2,
            )
        ...
        fig.update_layout(
            title_text=f"Pipeline Dashboard - Last {days} Days",
            height=1200,
            showlegend=True,
    ...
    def simulate_pipeline_data(self):
        ...
        for i in range(100):
                ...
                run_data["steps"].append(
                    {
                        ...
                        "status": step_status,
                        "start_time": (
                            timestamp + timedelta(seconds=step_start)
                        ).isoformat(),
                        "end_time": (
                            timestamp + timedelta(seconds=step_start + step_duration)
                        ).isoformat(),
                    }
                )
            ...
            if random.random() > 0.3:
                ...
                failed = max(0, total - passed - random.randint(0, 5))
...
def main():
    ...
    parser.add_argument(
        "--record-metrics", action="store_true", help="Record pipeline metrics"
    )
    parser.add_argument(
        "--generate-dashboard", action="store_true", help="Generate dashboard HTML"
    )
    parser.add_argument(
        "--simulate-data", action="store_true", help="Generate sample data for testing"
    )
    parser.add_argument(
    ...
    if args.record_metrics:
        run_data = {
            ...
            "status": (
                "success"
                if os.environ.get("BITBUCKET_EXIT_CODE", "0") == "0"
                else "failed"
            ),
            "duration": 0,
            "triggered_by": (
                "pull_request" if os.environ.get("BITBUCKET_PR_ID") else "push"
            ),
