...
class DependencyHealthCheck:
    ...
    def _parse_requirements(self) -> List[Dict[str, str]]:
        ...
        with open(self.requirements_file, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if len(match) == 3:
                    name, specifier, ver = match
                    version = ver.strip().split(",")[0].strip()
    ...
    def check_vulnerabilities(
        ...
        try:
            ...
            response = self.session.post(
                "https://api.osv.dev/v1/query", json=query, timeout=10
            )
            ...
            if version:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.warning(
                            "PyUp API requires authentication for %s — skipping (set PYUP_API_KEY to enable)",
                            package_name,
                        )
                    else:
                        logger.debug(
                            "PyUp API returned %d for %s",
    ...
    def calculate_health_score(
        self, package_info: Dict, vulnerabilities: List[Dict]
    ) -> Dict:
        ...
        if not package_info:
            return {"score": 0, "reasons": ["No package info available"], "details": {}}
                ...
                if files and "upload_time" in files[0]:
                    release_dates.append(
                        datetime.fromisoformat(
                            files[0]["upload_time"].replace("Z", "+00:00")
                        )
                    )
        ...
        try:
            ...
            if downloads is None:
                response = self.session.get(
                    f"https://pypistats.org/api/packages/{pkg_name}/recent", timeout=5
                )
        ...
        if vulnerabilities:
            ...
            reasons.append(
                f"Found {vuln_count} vulnerabilities ({critical} critical, {high} high)"
            )
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_package):
                ...
                try:
                    ...
                    results.append(
                        {"package": package["name"], "error": str(e), "health_score": 0}
                    )
    ...
    def generate_report(self, results: List[Dict]) -> str:
        ...
        for result in results:
            score_class = (
                ...
                else "score-1" if result["health_score"] < 70 else "score-2"
            )
        ...
        if needs_update > 0:
            recommendations += (
                f"<li>Update {needs_update} packages to latest versions</li>"
            )
    ...
    def check_for_failures(self, results: List[Dict], thresholds: Dict = None) -> bool:
        ...
        for result in results:
            ...
            if critical > thresholds.get("critical_vulnerabilities", 0):
                logger.error(
                    "%s: Found %d critical vulnerabilities", result["package"], critical
                )
                failed = True
...
def main():
    ...
    parser.add_argument(
        "--check-all", action="store_true", help="Check all dependencies"
    )
    parser.add_argument(
        "--generate-report", action="store_true", help="Generate HTML report"
    )
    parser.add_argument(
        "--fail-on-issues", action="store_true", help="Fail if issues found"
    )
    ...
    if args.check_all:
        ...
        for result in results:
            status = (
                ...
                else "WARN" if result["health_score"] >= 50 else "FAIL"
            )
            vuln = (
